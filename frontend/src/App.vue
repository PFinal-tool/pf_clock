...
// 创建 refs 来保存当前的小时、分钟和秒
const hours = ref(0);
const minutes = ref(0);
const seconds = ref(0);
let timer = null; // 保存计时器的引用
...
<template>
  <div class="clock">
    <div class="clock__unit" data-unit="h">
      <div class="clock__digits">{{ String(hours).padStart(2, '0') }}</div>
    </div>
    <div class="clock__unit" data-unit="m">
      <div class="clock__digits">{{ String(minutes).padStart(2, '0') }}</div>
    </div>
    <div class="clock__unit" data-unit="s">
      <div class="clock__digits">{{ String(seconds).padStart(2, '0') }}</div>
    </div>
  </div>
   <!-- 按钮 -->
  <div class="controls">
...
.clock {
  ...
  box-shadow: 0 0 0.5em hsla(var(--hue), 10%, 10%, 0.5), 0 0 0.0625em hsla(var(--hue), 10%, 10%, 0.5) inset;
  display: flex;
...
.clock__digits {
  ...
  box-shadow: 0 0 0 1px hsl(var(--hue), 10%, 10%, 0.05) inset, 0 0 0.0625em hsl(var(--hue), 10%, 10%, 0.3) inset;
...
.clock__unit {
  ...
  box-shadow: 0 0 0 0.0625em hsl(0, 0%, 0%), 0 -0.0625em 0.125em 0.125em hsla(var(--hue), 10%, 10%, 0.5), 0 0.0625em 0.125em 0.125em hsla(var(--hue), 10%, 90%, 0.8);
  perspective: 10em;
  position: relative;
  width: 4em;
  height: 4em
